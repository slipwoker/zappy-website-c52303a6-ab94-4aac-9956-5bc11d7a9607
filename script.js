document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    const VALIDATORS = {
      ...
      tel: (value) => /^[\d\s\+\-\(\)]{7,20}$/.test(value.trim()),
    ...
    function getOrCreateError(field) {
      ...
      error.style.cssText = 'display:block;color:#e53e3e;font-size:0.85em;margin-top:4px;';
    ...
    function validateField(field) {
      ...
      if (field.hasAttribute('required') && !VALIDATORS.required(field.value)) {
        ...
        isValid = false;
        errorType = 'email';
      } else if (field.type === 'tel' && field.value.trim() !== '' && !VALIDATORS.tel(field.value)) {
        isValid = false;
        errorType = 'tel';
      } else if (field.hasAttribute('minlength') && field.value.trim() !== '' && !VALIDATORS.minlength(field.value, field.getAttribute('minlength'))) {
        isValid = false;
        errorType = 'minlength';
      }
      ...
      if (!isValid) {
        errorEl.textContent = getErrorMessage(field, errorType);
  ...
  const animationTargets = document.querySelectorAll(OBSERVED_SELECTOR);
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'c52303a6-ab94-4aac-9956-5bc11d7a9607',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'c52303a6-ab94-4aac-9956-5bc11d7a9607';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'c52303a6-ab94-4aac-9956-5bc11d7a9607';
